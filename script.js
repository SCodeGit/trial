...
hub.addEventListener("mousedown", (e) => { 
  if(e.target === toggle || e.target === hub){ // allow drag by hub or toggle
    isDragging = true;
...
document.getElementById("scOpenAI").addEventListener("click", () => {
  ...
  if(ai){
    window.open(`https://www.${ai}.com`, "_blank");
  } else {
    alert("Select an AI first.");
  }
});
...
searchBtn.addEventListener("click", () => {
  const uni = universitySel.value,
        lvl = levelSel.value,
        sem = semesterSel.value,
        prog = programSel.value;
  
  if(!uni || !lvl || !sem || !prog){
    alert("Select all filters first.");
    return;
  }
